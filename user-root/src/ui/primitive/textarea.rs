...
#[component]
pub fn Textarea(
    #[prop(optional)] default_value: Option<impl IntoView>,
    ...
    match default_value {
        Some(node) => view! { <textarea class=class>{node.into_view()}</textarea> }.into_any(),
        None => view! { <textarea class=class /> }.into_any(),
    }
}
